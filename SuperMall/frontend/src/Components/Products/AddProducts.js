...
const AddProducts = () => {
  ...
  const onChange = (e) => {
    ...
    if (e.target.name === "shopName") {
      ...
      setProductValues({ ...productValues, mobile, floor, shopName: e.target.value });
    }
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const productAdded = await addProduct(productValues);
    if (productAdded) {
  ...
  const handleImageUpload = async (e) => {
    ...
    setProductValues({ ...productValues, productImage: imageUrl });
  };

  return (
    <div className="add-products-container">
      <form onSubmit={handleSubmit} className="login-form">
        <h1 className="form-heading">Add Products</h1>
        ...
        <div className="btn-container">
          <button type="submit" className="btn form-btn" onClick={handleReset}>
            Reset
          </button>
